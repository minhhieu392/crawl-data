...
/*
function makeArrayTemplateGroupsSearch(array, arrTreeId){
  return new Promise( async (resolve,reject) =>
  {
    try 
    {
     ...
     await Promise.all(array.map(async child =>
        {
            if(!_.includes(arrTreeId, child.id)) {
              ...
              const dataListChild=  await Model.findAll(templateGroups,
              {
              ...
              await makeArrayTemplateGroupsSearch(arrayChild,arrTreeId)
            }
        })
      )
...
function makeArrayMenusSearch(array, arrTreeId){
  return new Promise( async (resolve,reject) =>
  {
    try 
    {
     ...
     await Promise.all(array.map(async child =>
        {
            if(!_.includes(arrTreeId, child.id)) {
              ...
              if(child.parentId || Number(child.parentId) !==0)
              {
                const data=  await Model.findOne(menus,
                  {
                    where: {id:child.parentId},
                    attributes:['id','name','parentId'],
                  }
              ...
              const dataListChild=  await Model.findAll(menus,
              {
                  where: {parentId:child.id},
                  attributes:['id','name','parentId'],
              });
              ...
              await makeArrayMenusSearch(arrayChild,arrTreeId)
            }
        })
      )
  ...
  function makeArrayCategoriesSearch(array, arrTreeId){
  return new Promise( async (resolve,reject) =>
  {
    try 
    {
     ...
     await Promise.all(array.map(async child =>
        {
            if(!_.includes(arrTreeId, child.id)) {
              ...
              if(child.parentId || Number(child.parentId) !==0)
              {
                const data=  await Model.findOne(categories,
                  {
                    where: {id:child.parentId},
                    attributes:['id','name','parentId'],
                  }
              ...
              const dataListChild=  await Model.findAll(categories,
              {
                  where: {parentId:child.id},
                  attributes:['id','name','parentId'],
              });
              ...
              await makeArrayCategoriesSearch(arrayChild,arrTreeId)
            }
        })
      )
